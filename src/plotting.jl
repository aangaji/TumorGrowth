...
function plotting!(scene, data; path="", color=nothing, shading = false)
    isempty(data) && return
    p = (data isa DataFrameRow) ? [Point(data.position...)] : [Point(pos...) for pos in data.position]
...
function plotting(data; path="", color=nothing, shading=false, inline=false)
    AbstractPlotting.inline!(inline)
    scene = Scene()
    plotting!(scene, data; path=path, color=color, shading=shading)
    return scene
end
