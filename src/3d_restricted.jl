export birth_death_space_restricted_3D

#function to give birth to a new cell in the tumor (called with the tumor, the parental cell and the index of mutations)
function birth_space_restricted_3D(all_cells::DataFrame, cell::DataFrameRow, current_mut::Int64, mutation_rate::Float64, steps::Int64)

    pos_change = false
    position_new_cell = [0.0, 0.0, 0.0]
    count = 0
    ...
    while (pos_change == 0) & (count <= count_max)
        ...
        δ = 2 .* rand(3) .- 1 #normalized random vector
        δ = δ ./ norm(δ)
    ...
    if count == count_max+1
        return nothing, steps+1 #If the positioning was tried several times (count_max) and it didn't work: Abort!
    end
...
function birth_death_space_restricted_3D(; tumor_size::Int, b::Float64, d::Float64, mu::Float64)
    cells = DataFrame([[1], [[0.0,0.0,0.0]], [b], [d], [0], [[]]], [:index, :position, :birth_rate, :death_rate, :parental, :mutations])
    last_index = 0
    cur_mutation = 0
    count_distribution = []
    ...
    while length(cells[:,1]) < tumor_size
        ...
        if r <= b_prob #if birth is chosen
            ...
            new_born_cell = birth_space_restricted_3D(cells, chosen_cell, cur_mutation, mu, tumor_size) #give birth to a new cell
            ...
            deleterows!(cells, row_number_chosen_cell)
        end
