...
function tumor_stepper!(params, tumor, trange; b, d, μ, ρ=ρ)
    id, mut_id, t = params
    dim = length(first(tumor).position)
    time_series = Vector{DataFrame}()
    for tp in trange
        up = birth_death_pushing!(tumor, tp; b=b, d=d, μ=μ, ρ=ρ, cur_id=id, cur_mutation=mut_id, t=t, dim=dim)
        push!(time_series, deepcopy(DataFrame(tumor)) )
        id, mut_id, t = up[:index], up[:mutation], up[:time]
    end
    return time_series
end
function tumor_stepper(trange; b, d, μ, ρ, dim, seed = nothing)
    simresult = birth_death_pushing(first(trange); b=b, d=d, μ=μ, ρ=ρ, dim=dim, seed=seed)
    params = simresult |> r -> (r[:index], r[:mutation], r[:time])
    tumor_stepper!(params, simresult[:tumor], trange; b=b, d=d, μ=μ, ρ=ρ)
end
