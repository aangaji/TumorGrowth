...
function birth_death_pushing!( tumor::Vector{Cell}, mutation_events::Vector{Mutation_event}, tumor_size::Int64; b, d, mu, cur_id = 1, cur_mutation = 0, t = 0.0, dim=length(tumor[1].position), seed=nothing)
...
function birth_death_pushing!( tumor::Vector{Cell}, mutation_events::Vector{Mutation_event}, tend::Float64; b, d, mu, cur_id = 1, cur_mutation = 0, t = 0.0, dim=length(tumor[1].position), seed=nothing)
    ...
    #Juno.progress() do id
    prog = ProgressUnknown("Progress: t ")
    while t < tend
        N==0 && error("Tumor died")
        parent_row = rand(1:N)
        parent = tumor[parent_row]
...
function birth_death_pushing( t_or_size; b, d, mu, cur_mutation = 0, dim , seed=nothing)
    ...
    cur_id, cur_mutation, t = birth_death_pushing!(tumor, mutation_events, t_or_size; b=b, d=d, mu=mu, cur_mutation=cur_mutation, dim=dim, seed=seed)
